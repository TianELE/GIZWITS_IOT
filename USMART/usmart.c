 ...
 * @return  void
 */
void USMART_TIME_IRQHandler(void *arg)
{
 ...
 * @return  void
 */
static u8 usmart_timerid;	//USMART串口调试组件定时器使用ID

void usmart_init(u8 sysclk)
{
#if USMART_ENTIMX_SCAN==1
    //分频,时钟为10K ,100us中断一次,注意,计数频率必须为10Khz,以和runtime单位(0.1ms)同步.
	struct tls_timer_cfg timer_cfg;
    memset(&timer_cfg, 0, sizeof(timer_cfg));

    timer_cfg.unit = TLS_TIMER_UNIT_US;				//计时单位（us）
    timer_cfg.timeout = 100;						//溢出时间
    timer_cfg.is_repeat = TRUE;						//周期循环
    timer_cfg.callback = USMART_TIME_IRQHandler;	//定时器回调函数

    usmart_timerid = tls_timer_create(&timer_cfg);	//创建定时器

    if(WM_TIMER_ID_INVALID != usmart_timerid)
    {
        tls_timer_start(usmart_timerid);			//启动定时器
	}
	else
		printf("usmart_timerid creat fail ...\r\n");
