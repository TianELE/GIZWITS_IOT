...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagCTRL)
    {
        info->event[info->num] = EVENT_CTRL;
        info->num++;
        dataPoints->valueCTRL = gizStandardDecompressionValue(CTRL_BYTEOFFSET,CTRL_BITOFFSET,CTRL_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueCTRL != cur->valueCTRL)
    {
        GIZWITS_LOG("valueCTRL Changed\n");
        ret = 1;
    }
    ...
    if(last->valueALS != cur->valueALS)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueALS Changed\n");
            ret = 1;
        }
    }
    if(last->valueTEMP_S != cur->valueTEMP_S)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueTEMP_S Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    if(dataPoints->valueCTRL >= CTRL_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueCTRL Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(CTRL_BYTEOFFSET,CTRL_BITOFFSET,CTRL_LEN,(uint8_t *)devStatusPtr,dataPoints->valueCTRL);  
    }
    ...
    devStatusPtr->valueALS = exchangeBytes(gizY2X(ALS_RATIO,  ALS_ADDITION, dataPoints->valueALS)); 
    devStatusPtr->valueTEMP_S = exchangeBytes(gizY2XFloat(TEMP_S_RATIO,  TEMP_S_ADDITION, dataPoints->valueTEMP_S));     
